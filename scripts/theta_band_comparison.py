...
for file_name in file_list:
    ...
    dur = 1.0
    epochs = mne.make_fixed_length_epochs(raw_filtered, duration=dur, preload=True)
...
for label, power in theta_curves.items():
    plt.plot(power, label=label)

plt.title("Theta Band Power (4–7 Hz) Over Time")
plt.xlabel(f"Epoch ({dur}s each)")
